    ...
    constructor() {
        ...
        this.pageCount = function (url, limit) {
            ...
            fetch(url, {    method: 'get',
                            headers: {
                                        "x-api-key": '171d3563-d945-4e75-90f4-c545359d94fe',
                            },
                }).then(response => {
                        let totalPage = parseInt(response.headers.get("pagination-count") );
                        console.log('length : ', totalPage);
                        if (totalPage === 0) {       // create a page when no result is found
...
let defaultLoader = function () {
    ...
    let loader = function (event) {
        ...
        search.pageCount(`https://api.thecatapi.com/v1/images/search?limit=${limit}&order=${order}&mime_types=${type}&category_ids=${category}&breed_ids=${breed}`, limit);  // setting the pagecount 
    };
